...
namespace Auth.Services.Certificate
{
    public class KeyVaultCertificateService : ICertificateService
    {
        ...
        public X509Certificate2 GetCertificateFromKeyVault(string vaultCertificateName)
        {
            ...
            var certBundle = keyVaultClient.GetCertificateAsync(_vaultAddress, vaultCertificateName).GetAwaiter().GetResult();
            var certContent = keyVaultClient.GetSecretAsync(certBundle.SecretIdentifier.Identifier).GetAwaiter().GetResult();
