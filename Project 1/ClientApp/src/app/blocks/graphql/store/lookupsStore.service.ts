...
export class LookupsStoreService {
    ...
    public getStoredLookups(group): any[] {
        ...
        for (var j = 0, keys = Object.keys(store); j < keys.length; j++) {
            // if (keys[j].startsWith(`Lookup:{"groupKey":"${group}`)){
            //     console.log('match',keys[j]);
            // }else{
            //     console.log('do not match',keys[j]);
            // }

            if (keys[j].startsWith(`Lookup:{"groupKey":"${group}`) && this.client.cache["data"]["data"][keys[j]].groupKey == group)
                storedRecords.push({
                    generated: false,
                    id: keys[j],
                    __ref: keys[j],
    ...
    public getRootQueryLookups(group: string): any[] {
        ...
        // console.log('getRootQueryLookups',array_);
    ...
    readLookups(group): Lookup[] {
        ...
        try {
            ...
            const lookupsObj: any = this.client.readQuery({
                // query: lookupsByGroupQ,
                query:gql`
                query lookupsByGroup($group: String!){
                    lookupsByGroup(group: $group){
                        id
                        tenantId
                        groupKey
                        value
                ...
                console.log('readLookups',lookupsObj);
                
         let GeneratedObjectKey = this.GetObjectKey(group)

            // this.client.cache["data"]["data"][
            //     "ROOT_QUERY"
            // ][GeneratedObjectKey] = [{}];
        ...
        //   });
            ...
            if (lookupsObj && lookupsObj.lookupsByGroup) 
             lookups = lookupsObj.lookupsByGroup;
    ...
    writeFragment(lookup: Lookup): void {
        ...
        let id = `Lookup:{\"groupKey\":\"${lookup.groupKey}\",\"text\":\"${lookup.text}\"}`
    ...
    getSerializedFromStore(group: string): Lookup[] {
        ...
        console.log(StoredItems,RootQueryItems);
        ...
        if (StoredItems.length != RootQueryItems.length) {
            ...
            //     `,
            //     variables: { group: group },
            //     data: { lookupsByGroup: StoredItems }
            // });


            console.log(this.client.cache["data"]["data"]["ROOT_QUERY"][GeneratedObjectKey] );
            ...
            console.log('getSerializedFromStore',result);
        ...
        lookups.map((lookup: Lookup, index) => {
            ...
            if (FragmentsStore[`Lookup:${lookup.groupKey}:${lookup.text}`]) {
                ...
                newLookupsRef.push({
                    generated: false,
                    id: `Lookup:${lookup.groupKey}:${lookup.text}`,
                    __ref: `Lookup:${lookup.groupKey}:${lookup.text}`,
                    type: "id",
                    typename: "Lookup"
                });
            }
