...
export class deleteTagGqlCallback {
    ...
    public static update(proxy_, ev) {
        ...
        const idToRemove = "Tag:" + ev.data.deleteTag.id;
        
        this.deleteTag(proxy, idToRemove);
    }

    private static deleteTag(proxy: InMemoryCache, id: string) {
        try {
            proxy.evict({id: id});
            proxy.gc();
        } catch (error) {
