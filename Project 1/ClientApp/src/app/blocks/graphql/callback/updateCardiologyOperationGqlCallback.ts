...
import { CardiologyCondition, Patient, UpdateCardiologyOperationMutation } from './../generated/gqlServices';
import { patientQ } from './../gqlQueries';

export class updateCardiologyOperationGqlCallback {

    public static optimisticResponse(variables):UpdateCardiologyOperationMutation  {
        const response = {
            __typename: "Mutation",
            updateCardiologyOperation: variables
        };
        return response as UpdateCardiologyOperationMutation ;
    }

    public static update(proxy, ev, patientId, conditionId, replacedConditionId) {

        // @ If errors exists, do nothing
        if (ev.data.errors && ev.data.updateCardiologyOperation === null)
            return;

        let operation = Object.assign({}, ev.data.updateCardiologyOperation)

        // @ update Cardiology operation in patient query
     ...
     * 
     * @memberOf updateCardiologyOperationGqlCallback
     */
