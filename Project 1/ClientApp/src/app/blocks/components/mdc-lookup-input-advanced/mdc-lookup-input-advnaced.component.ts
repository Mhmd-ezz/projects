...
import { LookupViewBase, LookupInputBase } from 'app/blocks/graphql/generated/bases';
...
import { catchError, debounceTime, distinctUntilChanged, tap, map, takeUntil } from 'rxjs/operators';
...
import { getLookupsState } from '@appStore/selectors';
@Component({
    selector: 'mdc-lookup-input-advanced',
    templateUrl: './mdc-lookup-input-advnaced.component.html',
    styleUrls: ['./mdc-lookup-input-advnaced.component.scss'],
})
export class MdcLookupInputAdvancedComponent implements OnInit {
    ...
    @Input()
    public set dataLinkSurgery(value: any) {
        this.list = value;
    }
    // @ parent inputs
        ...
        fromEvent(this.input.nativeElement, 'keyup')
            ...
            .subscribe((response) => {
                if (response && response.lookups) {
                    console.log('getLookupsState',response)
                    let paginate = this._paginator.paginate<LookupBase>(response.lookups, this.ctrl.value, 0, 10, 'text', false, this.searchOptionsDefaults)
 ...
 */
    ...
    addValueProcessor(): void {
        ...
        if (this.editIndex >= 0) {
            ...
            if (dupChecker) {
                ...
                this.save._elementRef.nativeElement.innerText = 'system_update_alt';
                this.oldLookup = null;
                this.editItemRiskFactor = -1;
                return;
            }
    ...
    pushToStore(lookup) {
        ...
        const lookupInput: LookupInputBase | any = {
    ...
    private findInLookups(text): Lookup | null {

        const lookup = this.lookupsSubject$.value.find(lookup => lookup.text === text);
