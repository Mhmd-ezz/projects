...
import { CardiologyCondition,  MediaFile, MediaFileInput, CardiologyFollowup } from '../../../../blocks/graphql/generated/gqlServices';
...
import { DataPartitionBase, CardiologyFollowupBase, MediaFileBase, MedicationBase } from 'app/blocks/graphql/generated/bases';
...
@Component({
    selector: 'new-cardiology-followup',
    templateUrl: './cardiology-followup-form.component.html',
    styleUrls: ['./cardiology-followup-form.component.scss']
})
export class CardiologyFollowupFormComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this._router.url.includes('edit')) {
            this.CompTitle = 'Edit Followup';
        } else {
            this.CompTitle = 'New Followup';
            this.followup.opened = new Date();
        }
        ...
        this._store.select(fromSelectors.createCardiologyFollowup)
            .pipe(
            ...
            .subscribe(data => {
                if(this.followup.id) {
                this.onCreateCardiologyFollowupCallback(data);
                }
 ...
 .subscribe((response) => {
                ...
                if (response && response.data) {
                    ...
                    combineLatest(this.idParam$, this.queryParams$, (followupId, queryParams) => ({ followupId, queryParams }))
                        .subscribe(pair => {
                            ...
                            const followup = this.getCardiologyFollowup(_followupId, _conditionid);
                            ...
                            {
                                this.followup.cardiologyClinicalExamination.leftInferior=true;
                                this.followup.cardiologyClinicalExamination.leftSuperior=true;
                                this.followup.cardiologyClinicalExamination.leftTransverse=true;
                                this.followup.cardiologyClinicalExamination.rightInferior=true;
                                this.followup.cardiologyClinicalExamination.rightTransverse=true;
                                this.followup.cardiologyClinicalExamination.rightSuperior=true;
                            }
                            // @ On update Cardiology Condition 
                            this._store.select(fromSelectors.updateCardiologyFollowup, { id: this.FollowupId })
                                .pipe(
                                    takeUntil(this._unsubscribeAll),
                                    filter(data => !!data)
                                )
                                .subscribe(data => {                                   
                                    if (this.followup.id) {
                                        this.onUpdateCardiologyFollowupCallback(data);
                                    }
    ...
    */
    ...
    private sendXhrProcessor() {
        ...
        if (this.followup.id) {
            ...
            if (this.conditionId && this.patient.id) {
                ...
                this._store.dispatch(fromActions.updateCardiologyFollowup({ patientId: this.patient.id, conditionId: this.conditionId, replacedConditionId: this.replacedConditionId, followup: followup }));
            ...
            this.followup.id = AppUtils.GenerateObjectId();
            this.FollowupId= this.followup.id
            this.followup.opened = new Date();
            const followup = Object.assign({}, this.followup);
            this._store.dispatch(fromActions.createCardiologyFollowup({ patientId: this.patient.id, conditionId: this.conditionId, followup: followup }));


        }
    }
    private onUpdateCardiologyFollowupCallback(response) {
        this.errors = [];
    ...
    private onCreateCardiologyFollowupCallback(response) {
        this.errors = [];
        this._formUtilsService.formSaved();
        this.replacedConditionId = undefined;
        AppUtils.SetFormPrestine(this.form);
        this.followup.id = response.id;
        if (this.followup.name! === response.name) {
            this.followup.name = response.name;
        }
        this.isDirty = false;
        this._formUtilsService.popup('Followup created.');
    }
    // ----------------
    //      MEDIA
    // ----------------
    ...
    getCardiologyFollowup(
        ...
        if (
            ...
            const condition: CardiologyCondition = this.patient.patientInfo.specialities.cardiology
            ...
            const followup: CardiologyFollowup =
