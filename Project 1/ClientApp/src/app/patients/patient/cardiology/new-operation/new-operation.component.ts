import { CardiologyOperationInput } from './../../../../blocks/graphql/generated/gqlServices';
...
import { createCardiologyOperationGqlCallback } from './../../../../blocks/graphql/callback/createCardiologyOperationGqlCallback';
import { CardiologyOperationBase, MediaFileBase, DataPartitionBase, CardiologyConditionBase, PatientBase, LocationBase } from './../../../../blocks/graphql/generated/bases';
...
export class CardiologyNewOperationComponent implements OnInit {
        ...
        this.onOperationChange
            ...
            .pipe(
                switchMap(x => {
                    ...
                    if (this.operation.id) {
                        ...
                        return this._createCardiologyOperationGQL.mutate(
                            { patientId: this.patient.id, conditionId: this.conditionId, operation: operation },
                            {
                                optimisticResponse: createCardiologyOperationGqlCallback.optimisticResponse(operation),
                                update: (proxy, ev) => createCardiologyOperationGqlCallback.update(proxy, ev, this.patient.id, this.conditionId)
                            }
                        );
                    }
            ...
            .subscribe(
                (response) => {
                    ...
                    if ('createCardiologyOperation' in response.data && response.data.createCardiologyOperation) {
                        this.errors = [];
                        this.replacedConditionId = undefined;
                        AppUtils.SetFormPrestine(this.form);
                        const operation = Object.assign({}, this.operation);
                        this.operation = AppUtils.deepMergeWith(operation, response.data.createCardiologyOperation);
                    ...
                    else if ('updateCardiologyOperation' in response.data && response.data.updateCardiologyOperation) {
                        this.errors = [];
                        this.replacedConditionId = undefined;
                        AppUtils.SetFormPrestine(this.form);
                        const operation = Object.assign({}, this.operation);
                        this.operation = AppUtils.deepMergeWith(operation, response.data.updateCardiologyOperation);
    ...
    */
    ...
    predictDiagnosis() {
        ...
        if (this.patient &&
            ...
            const conditionIndex = this.patient.patientInfo.specialities.cardiology.conditions.findIndex(obj => obj.id === this.conditionId);

            if (conditionIndex > -1) {
                const condition: CardiologyConditionBase = this.patient.patientInfo.specialities.cardiology.conditions[conditionIndex];


                if (condition.activities.followups.length) {
                    // @ try get latest followup that contains any diagnosis
                    const followup = condition.activities.followups.reduce((preF, curF) => {
