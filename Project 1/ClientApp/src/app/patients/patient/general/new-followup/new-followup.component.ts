...
import { DataPartitionBase, GeneralFollowupBase, LocationBase, MediaFileBase, MedicationBase } from 'app/blocks/graphql/generated/bases';
...
@Component({
    selector: 'new-followup',
    templateUrl: './new-followup.component.html',
    styleUrls: ['./new-followup.component.scss']
})
export class NewFollowupComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this._store.select(fromSelectors.createGeneralFollowup)
            .pipe(
                takeUntil(this._unsubscribeAll),
                filter(data => !!data)
            )
            .subscribe(data => {
                if (this.followup.id) {
                    this.onCreateGeneralFollowupCallback(data);
                }
                // @ On update General Condition
                this.OnUpdateGeneralFollowup();
        ...
        //                 let followup = Object.assign({}, this.followup);
        //                 followup.name = this.generatFollowupName();

        //                 return this._createGeneralFollowupGQL.mutate(
        //                     { patientId: this.patient.id, conditionId: this.conditionId, followup: followup },
        //                     {
        //                         optimisticResponse: createGeneralFollowupGqlCallback.optimisticResponse(followup),
        //                         update: (proxy, ev) => createGeneralFollowupGqlCallback.update(proxy, ev, this.patient.id, this.conditionId)
        //                     }
        //                 );
        //             }
        ...
        //             if (response.data.createGeneralFollowup) {
        //                 this.errors = [];
        //                 AppUtils.SetFormPrestine(this.form);
        //                 this.replacedConditionId = undefined;

        //                 // let followup = Object.assign({}, this.followup)
        //                 // this.followup.medications = AppUtils.resolveUniqMedications(this.followup, response.data.createGeneralFollowup) as MedicationBase[]
        //                 // this.followup = AppUtils.mergeForForms(followup, response.data.createGeneralFollowup)

        //                 this.followup.id = response.data.createGeneralFollowup.id;
        //                 if (this.followup.name! === response.data.createGeneralFollowup.name)
        //                     this.followup.name = response.data.createGeneralFollowup.name

        //                 this.snackBar.open("Followup created", 'CLOSE', {
        //                     panelClass: "m-24",
        //                     duration: 2000,
        //                 })
        //                 this.isDirty = false;
        ...
        //             else if (response.data.updateGeneralFollowup) {
        //                 this.errors = [];
        //                 AppUtils.SetFormPrestine(this.form);
        //                 this.replacedConditionId = undefined;
        //                 // let followup = Object.assign({}, this.followup)
    ...
    */
    ...
    private OnUpdateGeneralFollowup() {
        this._store.select(fromSelectors.updateGeneralFollowup, { id: this.followup.id })
            .pipe(
                takeUntil(this._unsubscribeAll),
                filter(data => !!data)
            )
    ...
    private onCreateGeneralFollowupCallback(response) {
        this.errors = [];
        this._formUtilsService.formSaved();
        AppUtils.SetFormPrestine(this.form);
        this.replacedConditionId = undefined;

        this.followup.medications = AppUtils.resolveUniqMedications(this.followup, response) as MedicationBase[];
        ...
        this.followup = AppUtils.mergeForForms(this.followup, response);
        this.isDirty = false;
    ...
    private sendXhrProcessor() {
        ...
        if (this.followup.id) {
            ...
            this.followup.id = AppUtils.GenerateObjectId();
            const followup = Object.assign({}, this.followup);
            this._store.dispatch(fromActions.createGeneralFollowup({ patientId: this.patient.id, conditionId: this.conditionId, followup: followup }));
