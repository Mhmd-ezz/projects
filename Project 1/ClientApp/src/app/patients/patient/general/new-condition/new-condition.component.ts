...
import { DataPartitionBase, GeneralConditionBase, LocationBase, MediaFileBase, MedicationBase, PatientBase } from './../../../../blocks/graphql/generated/bases';
...
export class NewConditionComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    public condition: GeneralConditionBase = new GeneralConditionBase();
    ...
    testApollo() {
        ...
        let condition = {
            "id": "62c04a8e6e12e8748e0d3547",
            "name": "test1",
            "type": "Condition",
            "status": "active",
            "subLocation": "",
            "opened": "2022-07-02T13:39:18.495Z",
            "closed": null,
            "location": {
                "id": "5c9e0a45b9cb6a1d4858eea5",
                "name": "Rasoul hospital",
        ...
        const id = '60f08af2d70bafa98f631f5c'
        // const client = getApolloClient();
        const q = gql`query patient($id: String!){
            patient(id: $id){
                id
                name
                gender
        ...
        const { patient } = this._apollo.client.cache.readQuery({ query: patientQ, variables: { id: id } });
        console.log(patient)
        let loadedpatient: PatientBase = patient
        ...
        if (index == -1) {
            // loadedpatient.patientInfo.specialities.general.conditions.push(condition as any)
            // loadedpatient.patientInfo.lastSeen = condition.opened ? condition.opened : new Date();
            // this._apollo.client.writeQuery({ query: patientQ, variables: { id: id }, data: { patient: loadedpatient } });
        }
    }
    ...
    ngOnInit(): void {
       // this.testApollo()
        ...
        this._store.select(fromSelectors.createGeneralCondition)
            .pipe(
                takeUntil(this._unsubscribeAll),
                filter(data => !!data)
            )
            .subscribe(data => {
                if (this.condition.id)
                    this.onCreateGeneralConditionCallback(data);
                // @ On update General Condition        

                this.onUpdateGeneralCondition();
            });
    ...
    private onUpdateGeneralCondition() {
        this._store.select(fromSelectors.updateGeneralCondition, { id: this.condition.id })
            .pipe(
                takeUntil(this._unsubscribeAll),
                filter(data => !!data)
            )
    ...
    private onCreateGeneralConditionCallback(response) {
        this.errors = [];
        ...
        // this.condition = AppUtils.mergeForForms(condition, response)
    ...
    private sendXhrProcessor() {
        ...
        if (this.condition.id) {
            ...
            const condition = Object.assign({}, this.condition);
            this._store.dispatch(fromActions.createGeneralCondition({ patientId: this.patient.id, condition: condition }));
        }
    }
