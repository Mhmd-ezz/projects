...
export class ProductService {
    ...
    public async productMaxPrice(maximum: any): Promise<any> {
     ...
     // product list
     public async customProductList(limit: number, offset: number, manufacturerId: number, categoryId: string, keyword: string, priceFrom: string, priceTo: string, price: string): Promise<Product[]> {
        return await this.productRepository.customProductList(limit, offset, manufacturerId , categoryId, keyword, priceFrom, priceTo, price);
    }
      ...
      public async customVendorProductList(limit: number, offset: number, status: number , vendorId: number, keyword: string, price: string): Promise<Product[]> {
        return await this.productRepository.customVendorProductList(limit, offset, status, vendorId, keyword,  price);
    }

    // Product count
    public async productCount(keyword: string, categoryslug: string, priceFrom: number, priceTo: number): Promise<any> {
        return await this.productRepository.productCount(keyword, categoryslug, priceFrom, priceTo);
    }
