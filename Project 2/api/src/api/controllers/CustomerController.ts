...
// Get Customer Detail API
    ...
    public async customerDetails(@Param('id')Id: number, @Res() response: any): Promise<any> {
        const customer = await this.customerService.findOne({
            select: ['id', 'firstName', 'email', 'mobileNumber', 'address', 'lastLogin'],
            where: {id: Id},
        });
        if (!customer) {
            const errorResponse: any = {
        ...
        customerDetails.productLists = await this.orderService.find({
        ...
        customerDetails.productViewedData = await this.productViewLogService.find({
