...
export class CustomerCouponController {
    ...
    public async applyCoupon( @Req() request: any, @Res() response: any): Promise<any> {
        ...
        const couponUsage = await this.couponUsageService.list(0, 0, [], [], [], 1);
        ...
        if (validCustomer) {
            ...
            console.log(customerUsagePerCoupon + 'customerUsagePerCoupon');
        ...
        if (validCode.allQualifyingItemsApply === 1) {
            ...
            const successResponse: any = {
                status: 0,
                message: 'Discounted product list',
                data: {vendorCouponId: validCode.vendorCouponId,
