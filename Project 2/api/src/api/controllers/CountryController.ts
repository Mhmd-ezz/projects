...
export class CountryController {
    ...
    public async addCountry(@Body({ validate: true }) countryParam: CreateCountry, @Res() response: any): Promise<any> {
        ...
        newCountry.isAllowed = countryParam.isAllowed;
    ...
    public async updateCountry(@Body({ validate: true }) countryParam: UpdateCountry, @Res() response: any): Promise<any> {
        const country: Country = await this.countryService.findOne({
            where: {
        ...
        country.isAllowed = countryParam.isAllowed;
        const countrySave = await this.countryService.update(country.countryId, country);
        if (countrySave !== undefined) {
            const successResponse: any = {
                status: 1,
                message: 'Successfully updated country',
    ...
    public async countryList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('keyword') keyword: string, @QueryParam('count') count: number | boolean, @Res() response: any): Promise<any> {
        const select = ['countryId', 'name', 'isoCode2', 'isoCode3', 'postcodeRequired', 'isActive', 'isAllowed'];
        const search = [
            {
                name: 'name',
                op: 'like',
    ...
    // Allowed Country List API
    /**
     * @api {get} /api/country/allowedCountries Country List API
     * @apiGroup Country
     * @apiHeader {String} Authorization
     * @apiParam (Request body) {Number} limit limit
     * @apiParam (Request body) {Number} offset offset
     ...
     *      "status": "1"
     * }
     * @apiSampleRequest /api/country/allowed-countries
     * @apiErrorExample {json} Country error
     * HTTP/1.1 500 Internal Server Error
     * @apiVersion 0.1.0
     */
    @Get('/allowed-countries')
    public async allowedCountriesList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('keyword') keyword: string, @QueryParam('count') count: number | boolean, @Res() response: any): Promise<any> {
        const select = ['countryId', 'name', 'isoCode2', 'isoCode3', 'postcodeRequired', 'isActive', 'isAllowed'];
        const search = [
            {
                name: 'name',
        ...
        if (countryList) {
            ...
            const errorResponse: any = {
                status: 0,
                message: 'unable to get allowed countryList',
            };
            return response.status(400).send(errorResponse);
        }
    }
