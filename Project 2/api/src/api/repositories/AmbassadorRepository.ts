...
@EntityRepository(Ambassador)
export class AmbassadorRepository extends Repository<Ambassador>  {

    public async getByUsername(username: string, isActive: boolean, deleteFlag: boolean): Promise<Ambassador> {

        const query: any = await this.manager.createQueryBuilder(Ambassador, 'ambassador');
        ...
        // query.select([]);
        query.innerJoin('ambassador.customer', 'customer');
    ...
    public async getList(limit: number, offset: number, select: any = [], searchConditions: any = [], whereConditions: any = [], count: number | boolean): Promise<any> {
        console.log(select);
        const query: any = await this.manager.createQueryBuilder(Ambassador, 'ambassador');
        // Select
       ...
       if (whereConditions && whereConditions.length > 0) {
        ...
        query.orderBy('ambassador.ambassador_id', 'DESC');
